...
class MySQLDataLayer(SQLAlchemyDataLayer):
    def __init__(
        self,
        conninfo: str,
    ...
    async def get_user(self, identifier: str) -> Optional[PersistedUser]:
        ...
        if result and isinstance(result, list):
            ...
            # MySQL returns JSON as string, we must convert it.
    ...
    async def get_thread_author(self, thread_id: str) -> str:
        ...
        query = """SELECT userIdentifier FROM threads WHERE id = :thread_id"""
        parameters = {'thread_id': thread_id}
        result = await self.execute_sql(query=query, parameters=parameters)
    ...
    async def delete_thread(self, thread_id: str):
        ...
        elements_query = """SELECT * FROM elements WHERE threadId = :thread_id"""
        elements = await self.execute_sql(elements_query, {'thread_id': thread_id})
        ...
        feedbacks_query = """DELETE FROM feedbacks WHERE forId IN (SELECT id FROM steps WHERE threadId = :thread_id)"""
        elements_query = """DELETE FROM elements WHERE threadId = :thread_id"""
        steps_query = """DELETE FROM steps WHERE threadId = :thread_id"""
        thread_query = """DELETE FROM threads WHERE id = :thread_id"""
        parameters = {'thread_id': thread_id}
        ...
        for thread in user_threads:
            ...
            if thread_id is not None:
                thread_dicts[thread_id] = ThreadDict(
                    ...
                    if thread.get('thread_metadata') is not None
                    else {},
                    steps=[],
                    elements=[],
                )
            ...
            for step_feedback in steps_feedbacks:
                ...
                if thread_id is not None:
                    ...
                    step_dict = StepDict(
                        ...
                        metadata=json.loads(step_feedback['step_metadata'])
                        ...
                        generation=json.loads(
                            step_feedback.get('step_generation', '{}')
                        ),
        ...
        if isinstance(elements, list):
            for element in elements:
                ...
                if thread_id is not None:
                    ...
                    thread_dicts[thread_id]['elements'].append(
                        element_dict
                    )  # type: ignore

        return list(thread_dicts.values())

    async def get_latest_round_image_per_user(
        self, start_timestamp: str, end_timestamp: str
    ) -> List[Dict[str, str]]:
        """Get the latest image generated by each user within a time range.
        ...
        Returns:
            List of dicts containing user identifier, image URL and creation timestamp
        """
        if self.show_logger:
            logger.info(
                f'SQLAlchemy: get_latest_round_image_per_user, '
                f'start={start_timestamp}, end={end_timestamp}'
            )

        query = """
            WITH latest_steps AS (
                SELECT t.userId, MAX(s.createdAt) AS latest_createdAt
                FROM steps s
                JOIN threads t ON s.threadId = t.id
                WHERE s.createdAt BETWEEN :start_ts AND :end_ts
                GROUP BY t.userId
            )
            SELECT
                u.identifier,
            ...
            JOIN steps s ON s.threadId = t.id AND e.forId = s.id
            JOIN users u ON u.id = t.userId
            JOIN latest_steps ls ON t.userId = ls.userId
                AND s.createdAt = ls.latest_createdAt
        """

        parameters = {'start_ts': start_timestamp, 'end_ts': end_timestamp}
    ...
    async def get_count_images(self) -> Dict[str, int]:
        query = """
            SELECT COUNT(*) FROM elements
        """
        result = await self.execute_sql(query=query, parameters={})
        count = result[0]['COUNT(*)']
        return {'images_count': count}

    async def get_round_images(self, start_timestamp: str, end_timestamp: str) -> List[Dict[str, str]]:
        query = """
            SELECT u.identifier, e.url, s.createdAt
            FROM elements e
            JOIN threads t ON e.threadId = t.id
            JOIN steps s ON s.threadId = t.id AND e.forId = s.id
            JOIN users u ON u.id = t.userId
